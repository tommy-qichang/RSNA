...
function test()
    ...
    for i = 1, len, bs do
        ...
        local predictResult = predictResult:reshape(predictResult:size(1)*predictResult:size(2),
        ...
        if(allResults)then
            allResults = torch.cat(allResults,predictResult,1);
            allTargets = torch.cat(allTargets,predictTarget,1);
        else
            allResults = predictResult;
            allTargets = predictTarget;
        end
    ...
    local rocData = {};
    rocData['oneresult'] = allResults:double();
    rocData['onetarget'] = allTargets:double();
